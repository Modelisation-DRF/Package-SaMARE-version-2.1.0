...
#' @param Para.ConvMSCRProd1024 Un dataframe  contenant les paramettres des équations de conversion du classement MSCR en produits pour les arbres de moins de 23.1 cm.
#' @param Para.ConvMSCRProd24 Un dataframe  contenant les paramettres des équations de conversion du classement MSCR en produits pour les arbres de plus de 23 cm.
#' @return La classe de produit de l'arbre.
#'
#' @export
AttribProd0 <- function(Data, Para.ConvMSCRProd1024, Para.ConvMSCRProd24) {
  ...
  if (Data$GrEspece %in% c("EPX", "RES", "SAB")) {
    ...
    if (!is.na(Data$Vigueur)) {
      ...
      if (!is.na(Data$MSCR) & Data$DHPcm < 23.1) {
        n <- nrow(Data)
