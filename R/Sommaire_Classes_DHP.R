#' Structure un dataframe de sortie dont chaque ligne correspond à la moyenne des itérations du nombre d'arbres par placette, groupe d'espèce, classe de DHP et année. La sortie présente donc un sommaire de la table de peuplement simulée pour chacune des placettes.
#'
...
#'
#' @return Un dataframe contenant le sommaire des itérations par placette, groupe d'espèce, classe de DHP et année.
#'
#' @export
Sommaire_Classes_DHP <- function(SimulHtVol, simplifier = FALSE) {
  ...
  if (simplifier == TRUE) {
    SommaireClassesDHP_simp_min <- SommaireClassesDHP %>%
      lazy_dt() %>%
      filter(Annee == MinAnnee) %>%
      as.data.frame()
    SommaireClassesDHP_simp_max <- SommaireClassesDHP %>%
      lazy_dt() %>%
