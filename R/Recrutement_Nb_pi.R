#' Prévoie la probabilité de présence de recrues par grouoe d'espèce.
#' Cette fonction corespond à la première portion de la fonction zero-inflated de prévision du nombre de recrues non basé sur les gaules.
#'
...
#' @param Para.rec_n Paramètres de l'équation de prévivion du nombre de recrues sans utiliser le nombre de gaules.
#'
#' @return  Le prédicteur linéaire de l'équation de la prévision de la présence de recrues par groupe d'espèce.
#'
#' @export
rec_pi <- function(Rec, t, st_tot0, ntrt, t0_aj_, type_pe_Plac, Iterj, Para.rec_n) {
  ...
  ParaRec_pi <- Para.rec_n %>% filter(Iter == Iterj & response == "pi")
