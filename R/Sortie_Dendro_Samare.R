#' Structure un dataframe de sortie pour lequels on rapporte pour chaque placette, annee et groupe d'espèce le diamètre quadratique moyen, la surface terrière, le volume et la hauteur dominante.
...
#'
#' @return  Un dataframe contenant par placette, groupe d'espèce, année et iteration la surface terrière le volume marchand brut, le diamètre moyen quadratique et la hauteur dominante.
#'
#' @export
SortieDendroSamare <- function(SimulHtVol, simplifier = FALSE) {
  ...
  suppressMessages(
    DendroSamaresp <- SimulHtVol %>%
      ...
      mutate(
        ...
        HDomM = ifelse(nbTi_HA > 100, mean(hauteur_pred[
          1:
          ifelse(
            length(first(which((NbCum / Sup_PE) >= 100))) == 0,
            first(which((NbCum / Sup_PE) >= 100)),
            0L
          )
        ], na.rm = TRUE), NA)
      ) %>%
      ...
      mutate(
        ST_HA = ifelse(is.na(ST_HA) == TRUE, 0, ST_HA),
        Vol_HA = ifelse(is.na(Vol_HA) == TRUE, 0, Vol_HA),
      ...
      mutate(DQM = (ST_HA / nbTi_HA / pi)^0.5 * 200) %>%
      ungroup() %>%
      as.data.frame()
  )
  ...
  if (simplifier == TRUE) {
    DendroIterSamare_simp_min <- DendroSamare %>%
      lazy_dt() %>%
      filter(Annee == MinAnnee) %>%
      as.data.frame()
