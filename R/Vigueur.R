#' Prévoit l'évolution de la classe de vigueur des arbres.
#' La vigueur peut prendre 2 valeurs soit "vigoureux" ou "non-vigoureux".
#'
#' @param Vig Un dataframe qui contient les arbres pour lesquels on veut prévoir l'évolution de la classe de vigueur des arbres.
...
#' @param Para.vig Un dataframe  contenant les parametres du module d'évolution de la vigueur des arbres.
#'
#' @return  Le prédicteur linéaire de l'équation de la probabilité que l'arbre soit de classe "vigoureux".
#'
#' @export
vig <- function(Vig, type_pe_Plac, rid1, Iterj, Para.vig) {
  ...
  ParaVigi <- Para.vig %>% filter(Iter == Iterj)
