#' Prévoit l'évolution de la classe de produit des tiges feuillus. Produit qui est contenu dans les classes "sciage" ou "pate".
#'
#' @param Prod Un dataframe qui contient les arbres pour lesquels on veut prévoir l'évolution de la classe de produit des arbres feuillus.
...
#' @param Para.prod Un dataframe  contenant les parametres du module d'évolution des produits des tiges feuillues.
#'
#' @return Le prédicteur linéaire de l'équation de la probabilité que la tige contienne un produit de type "sciage".
#'
#' @export
produit <- function(Prod, type_pe_Plac, rid1, Iterj, Para.prod) {
  ...
  ParaProdi <- Para.prod %>% filter(Iter == Iterj)
