...
SaMARE <- function(Random, RandomGaules, Data, Gaules, ListeIter, AnneeDep, Horizon, RecruesGaules,
  ...
  ######### Creation paramètres
  ParaTot <- map_dfr(seq_len(1), ~Para) %>% mutate(Iter = 1)

  ############## Gaules
  ParaTotGaules <- map_dfr(seq_len(1), ~ParaGaules) %>% mutate(Iter = 1)
  ...
  PlacQual <- Plac %>% filter(ABCD %in% c("A", "B", "C", "D") & Etat == "vivant")
  ...
  for (k in 1:Horizon) {
    ...
    if (RecruesGaules == 1) {
      ...
      # dataframe de base pour le recrutement
      ...
      RecTot <- RecBase %>%
        ...
        group_by(GrEspece) %>%
        # Assure d'avoir un ,aximum de 150 recrues
    ...
    if (k < Horizon) {
      ...
      ##### Enlève les arbres morts des résidus pour la prochaine step de croissance
      Residus <- matrix(Residus[m, ], nrow = length(m), ncol = Horizon + 1)
    }

    Plac <- Plac %>% rename(Etat = Etat1, DHPcm = DHPcm1, vigu0 = vigu1, prod0 = prod1)
