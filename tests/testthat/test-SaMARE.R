test_that("SaMARE returns the expected data frame without Gaules and MCH = 0", {
  set.seed(NULL)
  set.seed(3)
  ListeItertest <- data.frame(
    PlacetteID = "TEM23APC5000_1",
  ...
  actual <- SaMARE(
    Random = RandomTest,
    Data = Data_test_for_simul_samare,
    Gaules = NA,
    ListeIter = ListeItertest,
    Annee_Inventaire = 2023,
    Horizon = 6,
    RecruesGaules = 0,
  ...
  expected <- as.data.frame(readRDS(test_path("fixtures", "result_samare_sans_gaules_test.rds")))
  set.seed(NULL)
...
test_that("SaMARE returns the expected data frame without Gaules and MCH = 1", {
  ...
  expected <- as.data.frame(readRDS(test_path("fixtures", "result_samare_sans_gaules_test_MCH.rds")))
  set.seed(NULL)
  expect_equal(actual, expected)
})
...
test_that("SaMARE returns the expected data frame with Gaules MCH=0", {
  ...
  actual <- SaMARE(
    Random = RandomTest,
    RandomGaules = RandPlacStepGaules_test,
    Data = Data_test_for_simul_samare,
    Gaules = Gaules_test,
    ListeIter = ListeItertest,
    Annee_Inventaire = 2023,
  ...
  set.seed(NULL)
  expected <- as.data.frame(readRDS(test_path("fixtures", "result_samare_avec_gaules_test.rds")))
  expect_equal(actual, expected)
})
...
test_that("SaMARE returns the expected data frame with Gaules MCH=1", {
  ...
  set.seed(NULL)
  expected <- as.data.frame(readRDS(test_path("fixtures", "result_samare_avec_gaules_test_MCH.rds")))
  expect_equal(actual, expected)
})
