test_that("SaMARE returns the expected data frame without Gaules and MCH = 0", {
  ...
  result_simul2 <- SaMARE(
    Random = RandomTest, Data = Data_test_for_simul_samare, Gaules = NA, ListeIter = ListeItertest, AnneeDep = 2023,
    Horizon = 6,
...
test_that("SaMARE returns the expected data frame without Gaules and MCH = 1", {
  ...
  RandomTest <- readRDS(test_path("fixtures", "RandomTest.rds"))
  result_simul2 <- SaMARE(
    Random = RandomTest, Data = Data_test_for_simul_samare, Gaules = NA, ListeIter = ListeItertest,
...
test_that("SaMARE returns the expected data frame with Gaules MCH=0", {
  ...
  result_simul <- SaMARE(
    Random = RandomTest, RandomGaules = RandPlacStepGaules_test, Data = Data_test_for_simul_samare,
