...
test_that("simulateur SaMARE return the expected data with Gaules and coupe MCH=0", {
  ...
  expect_test_for_Simulateur_Samare <- expect_test_for_Simulateur_Samare %>%
    mutate(vol_dm3 = round(vol_dm3, 0)) %>%
...
test_that("simulateur SaMARE return the expected data with Gaules and coupe MCH=1", {
  ...
  expect_test_for_Simulateur_Samare_MCH <- expect_test_for_Simulateur_Samare_MCH %>%
    mutate(vol_dm3 = round(vol_dm3, 0), hauteur_pred = round(hauteur_pred, 1)) %>%
    dplyr::select(-milieu, -veg_pot, -essence_hauteur, -essence_volume, -step, -nb_tige) # enlever les variables qui étaient nécessaire seulement pour tarifqc
...
test_that("simulateur SaMARE return the expected data with gaules and without Gaules MCH=0", {
  ...
  expect_for_Samare_sans_gaules_et_coupe_test <- expect_for_Samare_sans_gaules_et_coupe_test %>%
    mutate(vol_dm3 = round(vol_dm3, 0), hauteur_pred = round(hauteur_pred, 1)) %>%
    dplyr::select(-milieu, -veg_pot, -essence_hauteur, -essence_volume, -step, -nb_tige) # enlever les variables qui étaient nécessaire seulement pour tarifqc
...
test_that("simulateur SaMARE return the expected data with gaules and without Gaules MCH=1", {
  ...
  expect_for_Samare_sans_gaules_et_coupe_test_MCH <- expect_for_Samare_sans_gaules_et_coupe_test_MCH %>%
    mutate(vol_dm3 = round(vol_dm3, 0), hauteur_pred = round(hauteur_pred, 1)) %>%
    dplyr::select(-milieu, -veg_pot, -essence_hauteur, -essence_volume, -step, -nb_tige) # enlever les variables qui étaient nécessaire seulement pour tarifqc
...
test_that("simulateur SaMARE return the expected data with Gaules and without coupe MCH=0", {
  ...
  expect_for_Samare_avec_gaules_et_coupe_test <- expect_for_Samare_avec_gaules_et_coupe_test %>%
    mutate(vol_dm3 = round(vol_dm3, -1), hauteur_pred = round(hauteur_pred, 0)) %>%
    dplyr::select(-milieu, -veg_pot, -essence_hauteur, -essence_volume, -step, -nb_tige) # enlever les variables qui étaient nécessaire seulement pour tarifqc
...
test_that("simulateur SaMARE return the expected data with Gaules and without coupe MCH=1", {
  ...
  expect_for_Samare_avec_gaules_et_coupe_test_MCH <- expect_for_Samare_avec_gaules_et_coupe_test_MCH %>%
    mutate(vol_dm3 = round(vol_dm3, -1), hauteur_pred = round(hauteur_pred, 0)) %>%
    dplyr::select(-milieu, -veg_pot, -essence_hauteur, -essence_volume, -step, -nb_tige) # enlever les variables qui étaient nécessaire seulement pour tarifqc
...
test_that("simulateur SaMARE return the expected data without Gaules and coupe MCH=1", {
  ...
  expect_for_Samare_avec_coupe_sans_gaules_test_MCH <- expect_for_Samare_avec_coupe_sans_gaules_test_MCH %>%
    mutate(vol_dm3 = round(vol_dm3, -1), hauteur_pred = round(hauteur_pred, -1)) %>%
    dplyr::select(-milieu, -veg_pot, -essence_hauteur, -essence_volume, -step, -nb_tige) # enlever les variables qui étaient nécessaire seulement pour tarifqc
